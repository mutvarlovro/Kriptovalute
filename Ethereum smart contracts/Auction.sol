    ...
    function finalize() public {
        ...
        if(judgeAddress != address(0)){
            require(msg.sender == judgeAddress || msg.sender == getHighestBidder(),
                "Only the highest bidder or judge can finalize when judge is present.");
        }

        require(outcome == Outcome.SUCCESSFUL, "Outcome is not successful.");

        payable(sellerAddress).transfer(contractBalance);
    }
    ...
    function refund() public {
        ...
        if(judgeAddress != address(0)){
            require(msg.sender == judgeAddress || msg.sender == sellerAddress,
                "Only seller or judge can initiate refund.");
        }

        require(outcome == Outcome.NOT_SUCCESSFUL, "Outcome is successful");

        require(getHighestBidder() != address(0), "No bids.");

        payable(getHighestBidder()).transfer(contractBalance);
    }
